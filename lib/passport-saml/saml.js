...
SAML.prototype.initialize = function (options) {
  ...
  // to be embedded as keyInfo in envelopped signed authNRequest
  if (!options.ownPublicCert) {
    options.ownPublicCert = false;
  }

  // true to add an envelopped xml-dsig on POSTed authNRequest
  if (!options.signAuthRequestWithPostBinding) {
    options.signAuthRequestWithPostBinding = false;
  }
...
SAML.prototype.generateAuthorizeRequest = function (req, isPassive, callback) {
  ...
  .then(function(){
    ...
    return Q(xmlbuilder.create(request).end());
  })
  .then(function(doc) {
    // no signature needed
    if (self.options.privateCert && self.options.signAuthRequestWithPostBinding && 
        self.options.ownPublicCert)
      return callback(null, enveloppedSign(doc, self.options));
    else
...
function enveloppedSign(xml, options) {
  var algo;
  if('sha256' === options.signatureAlgorithm)
    algo = 'http://www.w3.org/2001/04/xmldsig-more#rsa-sha256';
  else if('sha256' === options.signatureAlgorithm)
      algo = 'http://www.w3.org/2001/04/xmldsig-more#rsa-sha512';
   else
      algo = 'http://www.w3.org/2000/09/xmldsig#rsa-sha1';

  var sig = new xmlCrypto.SignedXml();
  sig.keyInfoProvider = {
    getKeyInfo: function (key) {
      return "<X509Data>"+ pemToXml(options.ownPublicCert) +"</X509Data>";
    },
    getKey: function (keyInfo) {
      return options.ownPublicCert;
    }
  };

  sig.addReference('/samlp:AuthnRequest');
  sig.signatureAlgorithm = algo;
